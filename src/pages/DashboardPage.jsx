...
export default function DashboardPage() {
  ...
  const currentMonthData = data.filter(t => t.month === now.getMonth() + 1)
    ...
    <div className={`bg-white border rounded-xl p-5 shadow-sm`}>
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className={`text-3xl font-bold mt-1 ${color}`}>{prefix}{Math.abs(value).toFixed(2)}</p>
        </div>
        <div className={`p-2 rounded-lg ${color === 'text-green-600' ? 'bg-green-100' : color === 'text-red-600' ? 'bg-red-100' : 'bg-indigo-100'}`}>
          <Icon size={20} className={color} />
        </div>
      </div>
      <p className="text-sm text-gray-500 mt-2">{MONTHS[now.getMonth()]} {now.getFullYear()}</p>
    </div>
  )
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <StatCard title="Income" value={totalIncome} icon={TrendingUp} color="text-green-600" />
            <StatCard title="Expenses" value={totalExpenses} icon={TrendingDown} color="text-red-600" />
            <StatCard
              title="Net Savings"
              value={net}
              icon={Wallet}
            ...
            <div className="bg-white border rounded-xl p-5 shadow-sm">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Entries This Month</p>
